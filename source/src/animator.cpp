...
Animation& Animator::CreateAnimation(std::string const& name,
	std::string const& textureName, sf::Time const& duration, bool loop)
{
	m_Animations.push_back(
		Animation(name, textureName, duration, loop));
...
Animation* Animator::FindAnimation(std::string const& name)
{
...
void Animator::Update(sf::Time const& dt)
{
	...
	float scaledTime = (m_CurrentTime.asSeconds() / m_CurrentAnimation->getDuration().asSeconds());
	int numFrames = m_CurrentAnimation->getFrames().size();
	...
	m_Sprite.setTextureRect(m_CurrentAnimation->getFrames()[currentFrame]);
}
...
void Animator::SwitchAnimation(Animation* animation)
{
	...
	if (animation != nullptr)
	{
		m_Sprite.setTexture(AssetManager::GetTexture(animation->getTextureName()));
	}
